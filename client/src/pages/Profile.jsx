...
const Profile = () => {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      if (profile.experienceYears)
        payload.experienceYears = Number(profile.experienceYears);
  ...
  const handleResumeUpload = async (e) => {
    ...
    if (resumeUrl.trim()) {
      ...
      setUploadMessage("Please choose a file or paste a resume link.");
      return;
    }
  ...
  const downloadResume = async () => {
    ...
    const safeFilename = `Resume_${user.name.replace(/\s+/g, "_")}`;

    if (user.resume.startsWith("data:") || user.resume.startsWith("blob:")) {
    ...
    try {
      const response = await fetch(user.resume, { mode: "cors" });
      ...
      window.open(user.resume, "_blank", "noreferrer");
    }
  };
      ...
      {user?.role === "candidate" && (
        <div className="card" style={{ marginTop: "24px" }}>
          ...
          <p style={{ color: "var(--text-gray)", marginBottom: "15px" }}>
            Upload a file or paste a hosted resume link. The latest one you save
          ...
          >
            <div style={{ display: "grid", gap: "10px", flex: 1 }}>
              <input
                type="url"
                placeholder="Paste external resume link (Google Drive, Dropbox, public URL, etc.)"
                value={resumeUrl}
                onChange={(e) => setResumeUrl(e.target.value)}
                style={{ width: "100%", padding: "10px" }}
              />
              <div style={{ color: "var(--text-gray)", fontSize: "0.85rem" }}>
                Or choose a local file below.
              </div>
              <input
                type="file"
                accept=".pdf,.doc,.docx"
          ...
          {uploadMessage && (
            <p style={{ marginTop: "10px", color: "var(--primary-blue)" }}>
              {uploadMessage}
            </p>
          )}
          {user.resume && (
            ...
            >
              <p
                ...
                style={{ margin: 0, color: "#059669", fontWeight: 500 }}
              >
